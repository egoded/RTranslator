...
public class Global extends Application implements DefaultLifecycleObserver {
    ...
    public void setName(String savedName) {
        ...
        if(getBluetoothCommunicator() != null) {
            getBluetoothCommunicator().setName(savedName);
        }
    }
    ...
    /**
     * Возвращает папку с моделями (внешнее хранилище или fallback на внутреннее).
     */
    public File getModelsDir() {
        File ext = new File(Environment.getExternalStorageDirectory(), "RTranslator/models");
        if (!ext.exists()) ext.mkdirs();
        if (ext.exists() && ext.canWrite()) return ext;

        File internal = new File(getFilesDir(), "models");
        if (!internal.exists()) internal.mkdirs();
        return internal;
    }

    /**
     * Проверяет наличие всех моделей.
     */
    public boolean areModelsDownloaded() {
        ...
        for (String model : requiredModels) {
            if (!new File(ext, model).exists() && !new File(internal, model).exists()) {
                return false;
            }
        }
        return true;
    ...
    public long getAvailableInternalMemorySize() {
        ...
        if(internalFilesDir != null) {
            return new File(internalFilesDir.getAbsoluteFile().toString()).getFreeSpace() / 1000000L;
        }
        return -1;
    }
    ...
    public long getAvailableExternalMemorySize() {
        ...
        if(externalFilesDir != null) {
            return new File(externalFilesDir.getAbsoluteFile().toString()).getFreeSpace() / 1000000L;
        }
        return -1;
    }
